...
Implement the client and admin parts of a small e-commerce site according to the Phase 2 rubric: product catalog with filtering/pagination, product detail and cart, checkout with order persistence, and an admin area with product CRUD and image management.
...
- External libraries: Bootstrap is used for quick, responsive UI scaffolding and consistent styling. Blade templates are used for server-side rendering to keep the stack simple and focused.
- We did not add a full permissions package (for example Spatie) to keep the implementation aligned with the rubric and reduce integration complexity.
- Cart behavior note: when a user logs in, the session cart takes precedence over any previously saved DB cart (the app does not automatically merge carts). This was a deliberate simple strategy to keep behavior predictable.
